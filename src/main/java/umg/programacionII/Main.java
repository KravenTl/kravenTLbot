...
import umg.programacionII.BotTelegram.Bot;
import umg.programacionII.BotTelegram.BotRegistra;
import umg.programacionII.BotTelegram.PockemonBot;
import umg.programacionII.BotTelegram.botPregunton;
import umg.programacionII.model.User;
import umg.programacionII.service.UserService;
...
public class Main {
    ...
    private static void PruebaInsertaUsuario() {
        ...
        try {
            ...
            System.out.println("Valiendo madre");
            e.printStackTrace();
        }
    }
    ...
    private static void PruebaBuscarUsuarioPorId(int id) {
        UserService userService = new UserService();
        try {
            // Suponiendo que el ID de usuario es 1
            User user = userService.getUserByidusuario(id);
            if (user != null) {
                System.out.println("Usuario encontrado: " + user.getNombre());
                System.out.println("User email: " + user.getCorreo());
            } else {
                System.out.println("Usuario no encontrado!");
            }
        } catch (SQLException e) {
            System.out.println("Error al buscar el usuario por medio de su Id");
            e.printStackTrace();
        }
    }

    private static void PruebaBuscarUsuarioPorcarnet(String carnet) {
        UserService userService = new UserService();
        try {

            User user = userService.getUserBycarnet(carnet);
            if (user != null) {
                System.out.println("Usuario encontrado: " + user.getNombre());
                System.out.println("User email: " + user.getCorreo());
            } else {
    ...
    public static void main(String[] args) {
       ...
       /*PruebaInsertaUsuario();*/
        ...
        //PruebaBuscarPorId
        //PruebaBuscarUsuarioPorId(100);

        //PruebaBuscarPorcarnet
        //PruebaBuscarUsuarioPorcarnet("0905-12-12345");
        try {
            ...
            PockemonBot poke=new PockemonBot();
